 ...
 */
// Usa onObjectFinalized diretamente e acessa os dados via event.data (tipo StorageObjectData inferido)
export const processUploadedDocumentForAnalysis = onObjectFinalized(async (event: StorageEvent): Promise<null> => {
  if (!event.data) {
    logger.warn("Objeto do evento de Storage ausente no event.data.");
    return null;
  }

  const object = event.data; // event.data é StorageObjectData por inferência
